...
|---|---|---|
| **D1** | Review surface upgrade (Streamlit) | **BUILT (2026-08-07)** after a flow-prototype design approval. `review/dashboard.py` (Streamlit) is a thin view over `review/service.py` (offline-tested, no Streamlit import), which adapts the already-tested tracking/gates modules — so all invariants hold through the same guard (I2), Gate 3 only *records* the human submission with **no employer-submit path (I1, audited)**, approval locks versions (I4). Streamlit is an **optional** dependency (`pip install streamlit`; CI + the offline suite run without it). The spreadsheet/CLI + text digest remain the fallback surface. |
...
| **D6** | Claude model pin + API budget cap | **Real client wired & gated for BOTH scoring (F2) and tailoring (F4) — inert until opt-in.** `common/llmclient.AnthropicClient` makes structured-JSON Messages calls; `make_llm_client()` / `tailoring.llm_tailor.make_tailor()` return the real client only when a model is pinned in `settings.yaml` AND `spend_authorized: true` AND `ANTHROPIC_API_KEY` is present — otherwise the offline mock. The real tailor's output still passes the I3 claim validator. Nothing spends by default. To activate: set `llm.model_id`, flip `llm.spend_authorized`, provide the key, `pip install anthropic`. |
