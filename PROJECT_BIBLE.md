...
| **D6** | Claude model pin + API budget cap | **Real client wired & gated (inert until opt-in).** `common/llmclient.AnthropicClient` implements structured-JSON Messages calls; `make_llm_client()` returns it only when a model is pinned in `settings.yaml` AND `spend_authorized: true` AND `ANTHROPIC_API_KEY` is present — otherwise the offline mock. Nothing spends by default. To activate: set `llm.model_id`, flip `llm.spend_authorized`, provide the key, `pip install anthropic`. |
